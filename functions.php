...
function spotify_template_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'ecommerce-template' ),
		)
	);
...
function spotify_template_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'ecommerce-template' ),
...
function spotify_template_scripts() {
	...
	wp_enqueue_script( 'ecommerce-template-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
